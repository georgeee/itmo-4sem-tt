...
class Monad m => MonadThunkState s ref m | ref -> s where
...
newThunk :: MonadThunkState s ref m => Thunk ref -> m ref
newThunk th = do --thId <- nextThunkId
                 -- @TODO uncomment
                 --th' <- computeThunkFV th
                 let th' = th
                     -- @TODO replace 0 with thId
                 addThunk th' { thId = 0, thCounter = 1, thNormalized = Nothing }
